...
- **SRD sessions not counted as user-involvement.** Jones names SRD as requiring engagement of development team + user representatives + top-gun security expert (Chapter 7). For Internet-facing / privileged-data applications, SRD is a security-specific elicitation form that adds to the 12-form portfolio. Cross-link `security-officer-requirements-and-inspection`; user effort for SRD counts toward the 10–50% involvement ratio.
