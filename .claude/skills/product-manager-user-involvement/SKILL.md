...
Software projects fail user satisfaction tests in two ways: by skipping user participation altogether, or by adopting a single form (e.g., "we have a Product Manager who talks to users") and assuming that covers all twelve forms Jones inventories. This skill lets the Product Manager design a portfolio of user-involvement activities sized to the application — embedded users for small Agile work, surveys and usability labs for mass-user products — with explicit ratios so the team can detect when user involvement is too thin to underwrite satisfaction.
...
- Full traceability: `bibliography/skill-references.md` § `product-manager-user-involvement`.
