...
- **Reusability strategy without explicit security validation per artifact type.** The 15 artifact types include code, designs, test cases — all of which must carry security certification when sourced externally. The "two-edged sword" includes back doors, spyware, keystroke loggers (Jones Section 42). Cross-link `architect-reuse-certification` (security validation is one of the 11 supporting practices); for novel third-party artifacts touching security surfaces, dispatch `security-officer-threats-and-defenses` for vector check before admission.
