...
6. **Integrate with QA gates.** Test environments provisioned per pipeline stage; static analysis (cross-link `developer-static-analysis`) integrated; inspection results (cross-link `qa-inspections-program`) feed pipeline progression decisions. **For Internet-facing / privileged-data applications, dispatch `security-officer-testing-and-static-analysis` for secure-deployment controls: pre-deployment vulnerability scanning, signed-artifact verification, security rule subset in pipeline SAST, ethical-hacker engagement schedule per high-risk release. The Anthropic `security-guidance` plugin (a PreToolUse(Edit|Write|MultiEdit) regex hook for 9 unsafe-code patterns) is a usable in-IDE secure-coding reminder layer — wire it in `.claude/settings.json` for development sessions.**
...
- **Pipeline lacks secure-deployment controls.** Internet-facing / privileged-data applications without pre-deployment vuln scanning, without signed-artifact verification, without security-rule SAST in pipeline are exposed at the deployment surface (one of Jones's named threat surfaces, Section 42). Dispatch `security-officer-testing-and-static-analysis` for the controls; they are not the Developer's territory and not optional for in-scope classes.
