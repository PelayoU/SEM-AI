...
3. **Configure the rule set.** Start from the tool's default rule set; tune additions and suppressions to the project. **For security-relevant applications, configure the security rule subset in coordination with `security-officer-testing-and-static-analysis`; the security ruleset is tuned for the application's threat surface per `security-officer-threats-and-defenses`, and false-positive rate is managed separately from generic rules. Security-focused SAST contributes ~25% DRE on security defects alone (Jones Table 5-6), substantially higher in the combined stack.** Document each tuning with a one-line reason.
...
- **Security rules absent from the SAST configuration.** Security-focused static analysis is Jones BP #38 practice 7 (new code) and practice 8 (legacy code) — both forcing functions for in-scope classes. Generic SAST defaults rarely cover security-specific patterns adequately. For Internet-facing / privileged-data / safety-critical applications, the security rule subset comes from `security-officer-testing-and-static-analysis`; running generic SAST and calling it security coverage is a common defect.
