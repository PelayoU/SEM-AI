...
**The graph is read through five holistic product dimensions, complementing Functionality**: *Technology*, *UX design*, *Monetization*, *Acquisition*, *Offline experience*. These dimensions are the **lens every role uses to evaluate the impact of its own work** — an Architect's ADR touches multiple dimensions in its Consequences; a Developer's commit affects functionality + possibly UX (microcopy) + possibly technology (performance); a QA test class targets a specific dimension; a DevOps release rollout impacts acquisition / offline / monetization; a Security Officer's finding has cross-dimensional consequences. **PM, additionally, encodes the dimensions as structured slots** in the spine templates (vision, goal, capability, feature) because PM authors the spine — that is how the lens cannot be silently skipped at the planning level. Story and spec drop the dedicated slots: at that granularity the dimensions manifest as specific Acceptance check entries / Acceptance Criteria. **The same dimensions are also the risk surface of the feature**: an unvalidated dimension is a risk assumed without knowing — Monetization unvalidated = value risk, UX design unvalidated = usability risk, Technology unvalidated = viability risk, Acquisition + Offline unvalidated = business viability risk.

**The graph also has a vertical lens: the value chain Output → Outcome → Benefit → Value.** Each cycle of work traverses it — what was shipped (output), what changed in user / system behavior (outcome), what metric moved as a result (benefit), and whether the benefit was worth the cost (value). The links break independently: output without outcome (nobody adopted), outcome without benefit (theatre), benefit without value (negative ROI). The chain is **probabilistic, not deterministic** — even excellent teams complete it fully in only 10–30% of cycles. The framework therefore **enables value, it does not provide it**; its job is to create the conditions for value, not guarantee the result. **Learning is the guaranteed output of every cycle**, regardless of whether value materialized — a feature shipped, no adoption, clear understanding of why, is a successful cycle by the framework's standard. The Value chain section in the feature template (post-done) captures the chain honestly; the goal's Outcome statement lives at the outcome layer, not the output layer.

**The vision is the polar star; the features are the steps toward it.** The vision encodes both identity (Positioning) and ambition (Value ambition — the destination of the value chain, multi-dimensional). Features are the steps that navigate toward that ambition, and each feature carries one of two intents: **delivery** (produce value directly, the feature pays off learning already accumulated) or **experiment** (resolve an uncertainty about the path, the feature exists to produce learning). Both intents are first-class; the difference shows up in how the Value chain section is read post-done — for experiments, *Learning extracted* is the primary signal; for deliveries, *Value assessment* is. Experiments declare their primary unknown in the `Uncertainty addressed` slot of the feature template; pure deliveries leave that slot N/A. **Visibility from the outside: the engine MCP applies the native label `experiment` automatically when the slot is populated** — so the Issue list, the Projects v2 board, and `gh issue list` all distinguish experiments from deliveries at a glance without opening the Issue body. A healthy product cycle interleaves both — pure experiments never consolidate value; pure deliveries build blind on untested assumptions. Which dominates at any moment is the management's call, not a framework rule.
...
**Concepts that live outside the Issue Type catalog**, on GitHub's native objects:
...
**Shared concepts on every node** (each maps to a native Issue mechanism):
...
**The session is the project's conversation channel.** The bodies of the nodes (vision, goal, capability, feature, spec, …) are the *destilado* of conversation, not its substitute — they record what was decided and the condensed why, not the deliberation that produced them. The deliberation happens here, in the session: the human + the agent talking back and forth, exploring, validating, deciding. The session doc's Decisions section, the Issue comments the session leaves (📍 / ✅ / 🏁), and the transcript JSONL are the traces of that conversation; they are not the conversation itself, they are what survives it. An agent reading a node body to inform construction reads **the body's sections AND the Issue's comments** because the comments often carry context that the body summarises but does not fully capture. If a section reads as clear yet the work falters when implementation begins, the missing piece is in the conversation — go to the comments, open a session to clarify, or ask the human to surface what was said and not written down.
...
These comments form the chronological session history of the node, navigable from the Issue thread, surfaced by `get_node_artifacts(node)` as session-doc artifacts. No custom field, no in-body section to maintain.
