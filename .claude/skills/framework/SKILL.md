...
**Sourced, not improvised.** A skill's criteria come from audited bodies of knowledge (Cagan, Jones, Cohn, Patton, Gherkin, GISF). A skill states its method in full and names the framework that governs it — *Cagan's four product risks*, *Cohn's INVEST*, *Jones's risk inventory* — so it is operable as written, with no document to open. It carries no page or slide locators: full provenance lives once in `bibliography/skill-references.md`, the audit record, never inline. Apply the named framework; if a skill lacks a criterion you need, surface the gap — never invent authority. Improvisation also includes *not reaching for a skill that governs the work*: actively identify which skills apply — the implied ones, not only the obvious — and apply them in full; proceeding by feel where a skill exists is the same breach as inventing authority.

**Hold perspective; never tunnel.** A node never stands alone: changing it ripples along its `parent`, its `related`, its Map edges, and the cross-axis `adr`/`release` that bind it. When you create or change a node, walk those edges and **surface every consequence your change implies** — an orphaned child, an invalidated estimate, a `spec` now out of sync, a decision it contradicts. Surfacing is the obligation: you name and record the ripple; you do **not** silently execute the downstream fixes — that is the human's call or another role's, per *Working as roles*. The Map exists to make this check cheap; use it. An agent that authors the local node correctly but is blind to what governs it or to what it just broke has failed the framework, not satisfied it.
