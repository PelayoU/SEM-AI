...
2. **Build the inspection schedule** keyed to project milestones (cross-link `po-milestone-tracking`): requirements review milestone → requirements inspection; design milestones → architecture / external / internal design / DB design inspections; code milestone → code inspections; test plan / test case milestones → test plan and test case inspections; documentation milestone → user-doc inspection.
3. **Assign roles per session.** Moderator (often QA / SQA, or inspection moderator specialist per Table 9-23 #20: 1,000 FP scope, 35% defect removal impact). Recorder. Author. Reviewer(s). Coordinate participation with Architect, Developer, Designer, PO as appropriate.
...
- **Cross-reference: requirements defects need requirements inspections** — Jones BP #11 (depth in `po-requirements-discovery`).
