...
1. **Story-to-AC traceability** *(cross-reference to `product-manager-feature-decomposition`)* — every scenario maps to a story identifier. Convention: a story labelled `A` produces `AC-A1`, `AC-A2`, … Each AC carries the story letter so the audit trail is one-glance.
...
- Full traceability: `bibliography/skill-references.md` § `product-manager-spec-gherkin`.
