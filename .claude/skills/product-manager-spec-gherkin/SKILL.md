---
name: product-manager-spec-gherkin
...
A feature is only as buildable as its acceptance criteria are unambiguous. Gherkin — the syntax behind Cucumber and Specification by Example — turns the conversation from a story (`product-manager-feature-decomposition`'s 5 Cs) into a contract: a small set of *Given / When / Then* scenarios that the team, the user, and the test runner all read the same way. This skill lets the Product Manager author or refine that contract per feature, anchored in the Cucumber Gherkin reference and the Specification-by-Example practice, so QA can validate against it and Developer can implement against it without renegotiation.
...
1. **Story-to-AC traceability** — every scenario maps to a story identifier. Convention: a story labelled `A` produces `AC-A1`, `AC-A2`, … Each AC carries the story letter so the audit trail is one-glance.
2. **One Feature per `.feature` file (or per spec node)** — you can only have a single Feature in a `.feature` file. The Feature is named with the human capability, not the technical module.
3. **Free-form description follows `Feature:`** — the description states why this feature exists and which business rules it embodies. Description lines are ignored at runtime but available to readers and reporters.
4. **Background only when shared by every scenario** — Background is for steps that *every* scenario needs. If only some scenarios share the precondition, do not use Background.
5. **Scenarios use Given / When / Then** — Given establishes context, When is the action under test, Then is the observable outcome. *And* and *But* extend the previous keyword and read naturally.
6. **Observable outcomes only in Then** — *Then* names what the team can observe (state, output, side effect), not what the system "does internally". A *Then* that reads as implementation is the most common spec defect.
...
8. **Scenario Outline + Examples when the same scenario applies across multiple data sets** — the table-driven form is preferred over copying scenarios.
9. **Acceptance Criteria block at the head of the spec** — the spec node lists numbered AC (AC-A1, AC-A2, …) above the Gherkin block, each AC stated in one human sentence, then realised as one or more Scenarios below. Agree on what test confirms done *before* you build.
10. **All discovered scenarios covered: happy path + alternatives + error cases** — missing alternative paths is a chief source of defects escaping acceptance.
...
3. **Write the AC block first** — for each story letter under the parent feature, write one or more numbered AC sentences:
   ```
   ## Acceptance Criteria
   ...
   ```
4. **Translate each AC into Gherkin scenarios**:
   ```gherkin
...
7. **Apply secondary keywords sparingly**:
...
- **Multiple Features in one file/node.** Readers reject it instinctively. One feature per spec.
