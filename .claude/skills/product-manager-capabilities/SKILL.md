...
1. **Confirm the parent goal is active and SMART.** A capability beneath a fuzzy goal inherits the fuzziness; if the goal is weak, go back to `product-manager-goals` before continuing.
...
- Full traceability: `bibliography/skill-references.md` § `product-manager-capabilities`.
