...
- **Security-relevant capability scored without Security input.** For Internet-facing / privileged-data capabilities, the Cagan four-risks check (value / usability / feasibility / viability) includes security risk in feasibility and viability. Dispatch `security-officer-architecture` for feasibility (can we build it securely with current architecture?) and `security-officer-threats-and-defenses` for viability (does threat exposure compromise business viability?).
