...
Jones (p. 114) is explicit: *"Every major project should start by reviewing available benchmark information from either ISBSG or from other sources. Every process improvement plan should start by creating a quantitative baseline against which progress can be measured."* Yet most teams skip both. This skill lets the Product Owner establish baselines at project start, capture benchmark data at release close, and use both as the empirical defense layer for `po-cost-estimating` and `po-risk-analysis`. Without benchmarks, estimates are political fictions; without baselines, process-improvement is rhetoric.
...
1. **Term used precisely** *(Jones BP #31, p. 112)* — *benchmark* compares a project or organization against similar projects from other companies; *baseline* is a snapshot of quality and productivity at a specific time, used to evaluate progress during process improvement. Do not conflate.
2. **Function-point metrics, not LOC** *(Jones BP #31, p. 114)* — both ISBSG and Jones express data in function points (IFPUG and COSMIC variants supported). LOC-only metrics are not usable for cross-language comparison.
3. **Full benchmark covers Jones's 25 topics, partial covers the 10** *(BP #31, pp. 112–114)* —
   **Full (on-site, ~2 days):**
...
4. **ISBSG cross-check or explicit gap statement** *(BP #31, p. 114)* — ISBSG holds ~5,000 projects (growing ~500/year) but is *heavily weighted toward IT and web applications*; military, embedded, systems software, classified military, and applications above 10,000 FP are sparse or absent. The pass states whether ISBSG covers the class or notes the gap.
5. **Validation discipline matched to source** *(BP #31, p. 114)* — full on-site benchmarks include validation interviews with managers and team. ISBSG self-submission has *no formal validation*; account for that uncertainty when citing ISBSG comparisons.
6. **Function-point counting cost respected** *(BP #31, p. 114)* — full FP counting is slow (~400 FP/day per BP #6). For >10,000-FP applications use the high-speed methods named in BP #6 (pattern matching, light FP analysis) rather than skipping the FP metric.

## How you proceed

1. **Decide whether you need a benchmark or a baseline** *(BP #31, p. 112)*. External comparison → benchmark. Internal progress snapshot for SPI → baseline. Same 25 topics, different use.
2. **Pick full or partial.** Below the resources for a full ~2-day on-site collection, use the 10-topic partial via remote / survey. Recognize the limitation: partial benchmarks "lack the granularity for a full and complete statistical analysis" (Jones p. 114).
3. **Locate the source.**
...
5. **Validate where possible.** Cross-check FP count with another method (high-speed FP per BP #6); confirm defect counts against the QA defect log; sanity-check schedules against the project plan and milestone log.
...
7. **Use the benchmark as defense for estimates and risk analysis** *(BP #16, p. 80; BP #17, p. 82)*. Jones notes (p. 80): *"benchmarks are perceived as being more real than estimates"* — make this defense explicit when stakeholders push back on cost or schedule.
...
- **Skipping the FP count because it's slow.** Use the high-speed FP methods (BP #6) — pattern matching, light FP analysis — rather than abandoning FP for LOC.
...
- **Best Practice #31 — *Software Benchmarks and Baselines* (Capers Jones, *Software Engineering Best Practices*, McGraw-Hill 2010, pp. 112–115).** Benchmark/baseline distinction; 25-topic full inventory; 10-topic partial inventory; ISBSG ~5,000 projects, +500/year, IT/web-weighted, military classified absent; full on-site ~2 days vs remote partial ~2–3 hours; function-point variant support (IFPUG, COSMIC, Finnish, Netherlands); self-submission lack-of-validation caveat; "every major project should start by reviewing benchmark information" mandate.
- **Cross-reference for high-speed FP methods (pattern matching, light FP analysis)** — Capers Jones BP #6, pp. 51–52 (depth in `po-early-sizing`).
- **Benchmarks as defense for estimates** — Capers Jones BP #16, p. 80 (depth in `po-cost-estimating`).
- Full traceability: `bibliography/skill-references.md` § `po-benchmarks-baselines`.
