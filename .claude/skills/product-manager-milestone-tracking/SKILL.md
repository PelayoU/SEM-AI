...
- **Security milestones absent from the schedule.** For Internet-facing / privileged-data applications, security milestones layer on the canonical 13: SRD pass (after requirements review), security inspection pass per artifact (cross-link `security-officer-requirements-and-inspection`), security test pass + ethical-hacker pass (before customer acceptance), release security gate (cross-link `security-officer-security-program`). Mark `N/A — reason` for non-security-relevant releases; absent milestones without N/A reason is silent skipping.
