...
- **Axis 4 (quality attribute) scored without Security input.** Jones's axis 4 explicitly includes security vulnerability minimization alongside defect prevention, DRE, performance, and UX excellence. For Internet-facing / privileged-data applications, dispatch `security-officer-architecture` to evaluate methodology candidates against security needs (e.g., does the candidate support secure-coding standards? security inspections? SRD?). A methodology silent on security fails axis 4 and is unsuitable for in-scope classes.
