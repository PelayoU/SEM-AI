...
- **CR with security implications routed without Security review.** For CRs that change the attack surface (new external interface, new auth flow, new data class, new dependency), the CCB membership should include Security Officer or the CR is dispatched to `security-officer-threats-and-defenses` (vector impact) and `security-officer-requirements-and-inspection` (does it add or change `AC-S*` security AC?). Skipping the security review on a security-touching CR reintroduces vulnerabilities that SRD already cleared.
