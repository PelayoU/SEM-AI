...
7. **ISBSG submission planned** *(Jones BP #30, p. 111)* — for non-classified / non-proprietary applications, the ISBSG data collection tool is used from requirements through development; benchmark data is submitted at end. Cross-link to `po-benchmarks-baselines`.
...
3. **Set up effort + cost capture** *(measures 1–2)*. Granular enough to support WBS (cross-link to `po-project-planning`). Document the burden-rate assumption explicitly — Jones BP #30 p. 110 warns burden rates distort cross-company comparison.
4. **Set up milestone progress capture** *(measure 3)*. Cross-link to `po-milestone-tracking`. Milestones are formal review closures, not calendar dates.
...
6. **Set up requirements churn capture** *(measure 6)*. Monthly rate. Jones reports the band 0.5%–3% with average 1%–3% and cumulative up to 50%+ by deployment (BP #11). Cross-link to `po-change-control`.
...
- **No historical baseline.** Without baseline, process improvement cannot show progress. Cross-link to `po-benchmarks-baselines`.
