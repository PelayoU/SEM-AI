...
1. **Substantially bug-free, demonstrably** — the artifact has empirical evidence of approaching zero defects: inspection results, static-analysis clean reports, test execution history with defect counts, third-party certification certificates where available. Trust-by-reputation is insufficient.
2. **Free of viruses, spyware, keystroke loggers, back doors** — security validation is a separate precondition from functional defect-freeness. Jones is explicit that "reusable material, or at least source code, may have security flaws or even deliberate 'back doors' inserted by hackers, who then offer the materials as a temptation to the unwary".
3. **Eleven supporting practices present** — admission also requires:
...
4. **Central certification authority exists** — admission is governed by an authority, not the consuming team's individual judgement. Jones proposes an industry-funded nonprofit model, analogous to Underwriters Laboratories or Consumer Reports. Within an organization, this maps to an internal certification function (often the Architect role with QA support).
...
6. **Auditable certification trail for third-party reuse** — prefer vehicles with cryptographic signatures, published defect history, and security advisories over vehicles with marketing-only claims.
...
2. **Run the defect-freeness check.** Inspection log present? Static analysis clean? Test results with defect counts visible? If any is missing, the artifact is *not* admissible regardless of how attractive its functionality is.
3. **Run the security check.** Provenance verifiable? Cryptographic signature valid? Known-vulnerability database (CVE-equivalent) checked? "Back door" risk evaluated for low-reputation sources?
4. **Walk the eleven supporting practices.** Each must be present or explicitly waived with a reason that survives audit. The most commonly missing in practice are change records, distribution records, and warranties — and missing any of these creates an irreversible problem at first defect discovery.
...
- **Trust by reputation.** "It's a popular library / it's used everywhere / it's from a big vendor" is not certification. Certification must be demonstrable, not assumed.
- **Skipping the security check on third-party code.** Back doors and spyware in reusable code are real and increasing. Provenance + signature + CVE check are not optional.
...
- **Security validation handed to ad-hoc review.** The certification gate explicitly requires checks for back doors, spyware, keystroke loggers (Jones's two-edged-sword warning, Section 42). For third-party reuse from non-trivial sources (popular OSS, vendor libraries, SOA services), dispatch `security-officer-threats-and-defenses` for the security review and `security-officer-testing-and-static-analysis` for security-focused SAST on the candidate artifact. "Trust by popularity" is the named anti-pattern.
