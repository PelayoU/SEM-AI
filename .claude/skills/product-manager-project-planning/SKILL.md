...
- **Plan omits security work breakdown.** For Internet-facing / privileged-data applications, the WBS must include SRD (Security Requirements Deployment, ~2 weeks at project start), security inspections of requirements/specs (cross-link `security-officer-requirements-and-inspection`), security test stages (security testing + ethical hacking per `security-officer-testing-and-static-analysis`), and the release security gate (per release boundary). Dispatch `security-officer-security-program` for the security activity list; absent these allotments, security work compresses into "buffer" and disappears.
