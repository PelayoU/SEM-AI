...
3. **Pick the defect prevention activities.** JAD + QFD upstream + structured coding + complexity analysis are the high-impact starter set. Add TSP/PSP for new development at scale; add certified reuse (cross-link `architect-reuse-certification`) when reuse strategy is in place; add embedded users / test-first when methodology supports them. **For Internet-facing / privileged-data / safety-critical applications, dispatch `security-officer-security-program` to design the security prevention stack: SRD (Security Requirements Deployment) + 8 Jones security best practices (Section 38) + formal security inspections + secure-coding standards. The Security Officer carries 70% defect-prevention impact (Jones Table 9-23, the second-highest of any specialist after risk analysts).**
...
6. **Pick the testing set** (cross-link `qa-testing-strategy`). 3–12 forms typical, spread across developer / specialist / customer ownership. **For security-relevant applications, the security-specific stack is added via `security-officer-testing-and-static-analysis`:** security testing (~65% DRE on security defects), ethical hacking (~85% DRE — the highest single-method security DRE), security-focused static analysis (~25% DRE alone, much higher in combination). Jones notes security defects are among the hardest to eliminate; the cumulative security DRE projection must be computed separately and validated against the target (≥99% for mission-critical, ≥95% standard).
...
- **Security stack contribution missing from the cumulative projection.** Jones names security defects as among the hardest to eliminate (Table 5-6 commentary: "requirements defects, security defects, and defects in test materials are the most difficult to eliminate"). For security-relevant applications, the cumulative DRE projection must include the security-specific stack (SRD prevention, security inspections, security testing ~65%, ethical hacking ~85%, security-focused SAST ~25%). Dispatch `security-officer-testing-and-static-analysis` for the figures; assuming the generic stack covers security defects under-projects the leakage by an order of magnitude.
