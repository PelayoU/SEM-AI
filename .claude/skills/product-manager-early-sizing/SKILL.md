...
1. **Output in function points, not lines of code** — function points are best practice; lines of code is malpractice as a size metric. Code-line counts may appear as a secondary or tertiary metric but never as the primary size.
2. **Method matches the available information** —
...
4. **ISBSG cross-check or explicit gap statement** — when the application class is represented in ISBSG, the sizing carries an ISBSG comparison. When it is not (military classified, embedded niche), state that explicitly so downstream estimators do not over-trust the figure.
5. **Growth-rate prediction included** — pattern matching also predicts the rate at which requirements are likely to grow during development. The sizing output names both an initial FP and an expected growth band (compatible with the 1%–3%/month requirements-creep empirics).
...
4. **Cross-reference ISBSG (or its equivalent) where the class is represented.** Include comparable projects and their actual FP, schedule, and cost.
5. **Predict growth rate.** State the expected requirements-creep band (0.5%–3%/month) and accumulate it across the planned development duration.
...
- **Sizing in lines of code.** The lines-of-code approach is malpractice as a project size measure. Use FP. LOC is a code-density artifact, not a size measure for projects.
...
- **Ignoring growth.** A size produced today is not the size at delivery — cumulative growth of ~50% by deployment is typical for 10,000-FP applications. The size record must include the growth band.
