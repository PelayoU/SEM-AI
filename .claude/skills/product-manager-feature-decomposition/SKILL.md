...
- **Feature with security dimension decomposed without flagging security AC downstream.** When the feature touches Internet-facing surface, auth, crypto, privileged data, or input validation, the downstream spec must carry `AC-S*` security AC via dispatch to `security-officer-requirements-and-inspection`. Flag the security dimension at feature-decomposition time so the spec authoring (`product-manager-spec-gherkin`) knows to dispatch Security Officer for the `## Security AC` section.
