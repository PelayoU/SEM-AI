---
name: product-manager-risk-analysis
...
- **Category 9 (security flaws / vulnerabilities) and category 14 (external risks including cyberextortion) scored without Security input.** Jones names these as distinct risk categories that require security-specialist judgment, not generalist scoring. Dispatch `security-officer-threats-and-defenses` for the threat catalogue (17+ named vectors + 7 anti-patterns) and `security-officer-security-program` for the mitigation programme. Owner-role for security risks is **Security Officer**, not "team" or "PM".
