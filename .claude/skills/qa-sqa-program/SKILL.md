...
4. **Release-stop authority exists and the appeal path is defined** — formal SQA approval is a prerequisite for delivery on **quality grounds**. If SQA recommends against release due to quality issues, the recommendation can be overturned only by the division VP or the corporate president. The appeal path is documented before the first release decision is needed. **Modern hybrid (this framework's adopted model):** Security defects on **security grounds** flow through Security Officer's independent release-stop authority (`security-officer-security-program`), escalating to CISO / VP Security in parallel with SQA's quality-grounds veto. The two channels operate side-by-side; defects that are *both* security and quality (e.g., a buffer-overrun coding defect) are flagged through both. Jones's 2009 model lumps security into SQA's quality veto; this framework adopts the post-2009 hybrid because CISO-level governance is now industry practice.
...
6. **Define the release authority and the appeal path.** Formal SQA approval as prerequisite for delivery on quality grounds; appeal path through division VP / corporate president. **Coordinate with Security Officer** on the parallel security-grounds channel (`security-officer-security-program` release security gate); the two release-stop authorities operate in parallel under the modern hybrid model. Document both and brief executive leadership before the first release decision.
...
- **Security defects treated as quality-only defects.** In this framework's modern hybrid model, security defects on security-grounds flow through Security Officer's independent release-stop authority (escalates to CISO / VP Security), parallel to SQA's quality-grounds veto. Forcing security defects into the SQA quality channel only (Jones's 2009 model) is acceptable only when no Security Officer is staffed; otherwise it undermines the parallel-channel coverage. Defects that are both must be flagged in both channels.
