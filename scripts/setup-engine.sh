...
#
# Required externally (per ADR-009 update on dependencies):
#   - A CLI with MCP support (Claude Code, OpenCode, etc.) — for using the
#     engine; not verified here.
#   - `gh` CLI installed + authenticated
#   - Python 3.11+
#
# NOT required: mcp__github__* loaded by the CLI client. See ADR-009 update.
# ============================================================================
...
Next steps:
  ...
  Issue (per ADR-006 § Value ambition).
============================================================
EOF
